...
class HttpAdapter:
    ...
    def handle_client(self, conn, addr, routes):
        ...
        # Handle the request
        msg = conn.recv(2**16).decode()
        req.prepare(msg, routes)
        
        # Log the request: [method] path from address
        print("{} {} from {}".format(req.method or 'UNKNOWN', req.path or '/', addr))
    ...
    def handle_client_coroutine(self, conn, addr, routes):
        """
        Generator-based coroutine for handling client connections.
        """
        self.conn = conn
        self.connaddr = addr
        req = self.request
