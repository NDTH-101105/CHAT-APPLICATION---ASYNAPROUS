...
def resolve_routing_policy(hostname, routes):
    ...
    if isinstance(proxy_map, list):
        if len(proxy_map) == 0:
            ...
            proxy_host, proxy_port = proxy_map[0].split(":", 2)
            routes[hostname] = (proxy_map[1:] + [proxy_map[0]], policy) 
    else:
...
def handle_client(ip, port, conn, addr, routes):
    ...
    request = conn.recv(2**16).decode()

    # Extract method, path, and hostname
    method = 'UNKNOWN'
    path = '/'
    hostname = 'unknown'
    ...
    print("{} {} from {}".format(method, path, addr))
